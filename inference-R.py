...
##模型运行过程是完全分开的batch_size可以独立设置
# 面部识别batch_size
face_det_batch_size = 32
# 唇形转化batch_size
wav2lip_batch_size = 16
# 面部超分batch_size
gfpgan_batch_size = 16
# 面部粘贴batch_size
parse_batch_size = 16
...
def get_face_from_cache(video_path: str, frequency: str):
    ...
    if face_list and all_landmarks_5 and len(face_list) != len(all_landmarks_5):
...
def wav_to_lip_with_chin(
    ...
    :param with_sound: 是否合并音频
    :param start_index: 开始帧
    :param base_dir: 临时文件夹
    :param total_frame: 总帧数
    :param task_logger: 日志
    :param file_prefix: 文件前缀
    :return: 输出文件路径
    """
    if task_logger is None:
        task_logger = logger
    assert os.path.isfile(video_path), '视频文件不存在'
    # 推理模型的cv2 capture
    video_stream = cv2.VideoCapture(video_path)
    # 固定帧率
    ...
    # 缓存人脸存在直接获取，不存在就检测, 为什么要检测第一次和第二次?
    det_faces, all_landmarks_5 = get_face_from_cache(video_path, "once")
    # 没有检测到, 则重新检测并保存
    if not det_faces or not all_landmarks_5:
        md5_name = hashlib.md5(video_path.encode("utf-8")).hexdigest()
        ...
        with torch.no_grad():
            with torch.no_grad():
                ...
                for i in range(total):
                    if i != total - 1:
                        img = face_img[
                              i * face_det_batch_size:(i + 1) * face_det_batch_size]  # face_img.shape=(512,512,3)
                    elif i == total - 1:
    ...
    #command = f'ffmpeg -loglevel warning -y -i {tmp_file} -i {audio_path} -c:v libx264 -crf 18 -c:a aac -strict experimental -shortest {outfile}'
...
if __name__ == '__main__':
    a = time.time()
    print(wav_to_lip_with_chin("北影推理视频和音频/北影音频1.wav", "beiying3.mp4", "pt28.pth"))
    b = time.time()
    print("time", b - a)
